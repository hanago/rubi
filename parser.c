...
static Variable *appendVar(char *name, int type)
{
    if (functions.inside == IN_FUNC) {
        ...
        gblVar.var[gblVar.count].id = (int)&gblVar.data[gblVar.count];/* magic number */
...
{
    //uint32_t loopBgn = ntvCount;
    uint32_t loopBgn = npc++;// ntcCounter
    dasm_growpc(&d, npc);
    |=>loopBgn:
...
static int32_t functionStmt()
{
    ...
    espBgn = ntvCount + 2;
    emit(0x81); emit(0xe8 + ESP); emitI32(0); // sub esp 0 ; align8*/
    int func_addr = npc++;// ntcCounter
    dasm_growpc(&d, npc);
    int func_esp = npc++;// ntcCounter
    dasm_growpc(&d, npc);
    appendFunc(funcName, func_addr, func_esp, argsc); // append function
    |=>func_addr:
    | push ebp
    | mov ebp, esp
    | sub esp, 0x80000000 /* magic number */
    |=>func_esp:
...
int (*parser())(int *, void **)
{
    ...
    return ((int (*)(int *, void **))rubilabels[L_START]);/* magic code */
}
...
int32_t assignment()
{
    ...
    int n = (v->type == T_INT ? sizeof(int32_t) :
